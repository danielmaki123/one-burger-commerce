...
describe("buildOverviewRanges", () => {
  it("construye hoy como el día calendario completo de Managua", () => {
    const ranges = buildOverviewRanges("today", NOW);
  ...
  it("construye treinta fechas inclusivas terminando en la fecha local actual", () => {
    const ranges = buildOverviewRanges("30d", NOW);
    ...
    expect(buildOverviewBucketKeys(ranges)).toHaveLength(30);
  });

  it("compara el mes hasta el mismo ordinal de día", () => {
    const ranges = buildOverviewRanges("month", NOW);
...
describe("buildOverviewBucketKeys", () => {
  it("produce buckets horarios para hoy", () => {
    const buckets = buildOverviewBucketKeys(buildOverviewRanges("today", NOW));
