...
import { runPersistentAdmin } from "../../../../../../scripts/staging-persistent-admin-core";
...
function mapError(error: unknown) {
  if (error instanceof Error) {
    ...
    if (error.message === "STAGING_ADMIN_CONFIG_MISSING") {
      return NextResponse.json(
        {
          error: {
            code: "SERVER_MISCONFIGURATION",
            message: "Missing staging admin configuration",
          },
        },
        { status: 500 },
      );
...
export async function POST(request: Request) {
  try {
    ...
    const result = await runPersistentAdmin({
