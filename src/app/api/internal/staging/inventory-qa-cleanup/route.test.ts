...
const runStagingInventoryQaCleanupMock = vi.fn();

vi.mock("../../../../../../scripts/staging-inventory-qa-cleanup-core", () => ({
  runStagingInventoryQaCleanup: runStagingInventoryQaCleanupMock,
}));

describe("POST /api/internal/staging/inventory-qa-cleanup", () => {
  ...
  it("returns 403 when APP_ENV is not staging", async () => {
    ...
    const request = new Request("http://localhost/api/internal/staging/inventory-qa-cleanup", {
      method: "POST",
  ...
  it("returns 200 on successful cleanup", async () => {
    ...
    runStagingInventoryQaCleanupMock.mockResolvedValueOnce({
      status: "ok",
      source: "internal_api",
      precheck: { countExists: true, receiveExists: true, wasteExists: true, inventoryItemIds: [] },
      deleted: { count: 1, receive: 1, waste: 1 },
      postcheck: { countExists: false, receiveExists: false, wasteExists: false },
    });
    ...
    expect(runStagingInventoryQaCleanupMock).toHaveBeenCalled();
  });
