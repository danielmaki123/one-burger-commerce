...
export default function StockReceivePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) {
        ...
        setSubmitMsg("Sin permisos para registrar recepciones. Verifica tu sesión.");
        return;
      }
      if (!res.ok) {
        const errJson = await res.json().catch(() => ({}));
        setSubmitStatus("error");
        setSubmitMsg((errJson as { message?: string }).message || "No se pudo registrar la entrada. Intenta nuevamente.");
        return;
      }
      setSelectedItemId("");
      setQuantity(0);
      ...
      setSubmitMsg("Entrada de stock registrada exitosamente.");
    } catch {
      setSubmitStatus("error");
  ...
  return (
    <div className="space-y-8 pb-10">
      <div>
        <h1 className="font-heading text-3xl font-bold tracking-tight">Recepción de Stock</h1>
        <p className="text-muted-foreground">Registra la entrada de mercadería de proveedores.</p>
      </div>
      ...
      {fetchStatus === "auth" ? (
        ...
        <Card className="max-w-2xl">
          <CardHeader>
            ...
            <CardDescription>Ingresa los datos del producto recibido.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Input
                label={`Cantidad Recibida${selectedItem ? ` (${selectedItem.unit})` : ""}`}
                type="number"
                value={quantity}
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium leading-none text-foreground">Notas / Proveedor</label>
                <textarea
