...
> del roadmap: la **factura simple no fiscal** (modelo `Invoice` con migración aditiva, datos del negocio y
> del cliente **congelados** al emitir, número correlativo, sección «Factura» en el detalle del pedido y el
> papel impreso que el navegador guarda como PDF —misma decisión que el cierre de caja 1.6, sin dependencia
> nueva—) más las cuatro tareas del POS (**9.4/9.5** venta en espera, **9.6** cupones cotizados por el
> servidor, **9.7** descuentos manuales con permiso y firmados) y el arreglo del primitivo `Modal`.
> Verificado **en producción** con capturas reales del panel
> (`ops/tasks/audit-ui/tarea-9-*-produccion-*.png`, `tarea-factura-simple-produccion-1280.png`). El build
> anterior (`build-20260918-051408`, commit `9cf30f7`) llevaba 9.6/9.7. Lleva también
...
>
> **Ronda del roadmap POS (cerrada, 2026-09-18)**: las **8 tareas** que el owner confirmó están hechas y
> desplegadas — **1.2** (desglose por medio al cerrar), **1.5** (reporte diario de caja consolidado),
> **1.6** (el PDF del cierre se genera desde el navegador, sin dependencia nueva), **7.3** (helper E2E con
> rol `cashier`), **9.4/9.5** (venta **en espera**: dejarla a un lado y retomarla completa, con la clave
> del intento de cobro viajando con ella), **9.6** (cupones en el POS, cotizados por el servidor antes de
> cobrar) y **9.7** (descuentos manuales con permiso propio —owner y manager—, con motivo obligatorio y
> asentados en el log de acciones sensibles) — más la **factura simple no fiscal** del ítem 4. En el camino
> apareció y se arregló un bug del primitivo `Modal` (salía pegado a la esquina: el reset de Tailwind borra
> el `margin: auto` con el que el navegador centra el `dialog:modal`). **Decisión a confirmar por el
> owner**: la factura se imprime con la hoja del sistema y el navegador la guarda como PDF (igual que el
> cierre de caja); un PDF generado en el servidor necesitaría una dependencia nueva, que no se agrega sin
> aprobación. El **rediseño del menú público no se tocó** en esta ronda.
...
- **Hallazgo que cambia un pendiente**: producción **no tiene un local, tiene tres** (Camino de Oriente,
  Carretera Masaya y Casa Antigua) y **el owner confirmó que los tres son reales**, así que no se
  desactiva ninguno. Queda por corregir en `/admin/locations`: el slug de Camino de Oriente
  (`one-burger-masaya` → `camino-de-oriente`) y la ciudad de Casa Antigua, escrita `Jinoteoe`. Es
  cosmético y sin riesgo: el slug del local no se usa en ninguna URL pública ni se guarda en los pedidos
  (el público se maneja con `locationId`). Detalle en el runbook §8.8.
...
| 15 | **TASK-102: una sola puerta para el total** | **Cerrada el 2026-09-14** | `plna.md` §5. El total se calculaba a mano en varios lugares con variantes distintas. Ahora la fórmula vive solo en `calculateOrderTotal` (`src/shared/lib/order-totals.ts`), que usan el adaptador de Prisma, el de memoria, `calculateOrderTotals` y la tarjeta de resumen; hay un test de contrato que falla si alguien la reescribe. **Dos cosas quedaron afuera con motivo**: (a) el agregado de ítems de mesa (`add-table-order-items.ts:135-141`) calcula su total con su propia regla —fuerza empaque y propina en 0 y conserva el envío anterior— y no es una variante de la fórmula: unificarlo cambiaría el comportamiento de un pedido de mesa. (b) El checkout fija `discount: 0` en la estimación (`checkout/page.tsx:453-478`), así que un cupón no descuenta en el resumen que ve el cliente; es un hueco **preexistente** de la estimación (el monto real lo calcula el servidor al crear el pedido) y se ataca como task aparte. |
...
| 23 | **TASK-206: la pantalla de promos al design system (test de fuego)** | **Cerrada el 2026-09-14** | `plna.md` §5, FASE 2, cierre de la fase. `1df9ed8`: `/admin/promotions` pasa de **6 a 1** control crudo (queda la fila de lista, documentada: `Button` no cubre su anatomía) y de 629 a 601 líneas. Primitivo nuevo `src/shared/ui/select.tsx` con su test (TASK-201: 31 `<select>` crudos y `SELECT_CLASS` en 8 archivos; ahora 27 en `src/app` y 5 copias) y tamaño `pill` en `Button`. **Hallazgo del camino real**: en el CSS de Tailwind `.rounded-full` se emite **antes** que `.rounded-md`, así que un chip con `rounded-full` por `className` queda cuadrado en silencio; por eso el radio vive en el primitivo y el E2E mide el radio **calculado** en el navegador. Verificación: **276 archivos / 1877 tests**, lint, typecheck, `build`, `build:webpack` y `security:secrets` en verde; **E2E local 98 pasaron / 6 salteados / 0 fallos** (el caso nuevo mide chips y desplegables a 375 px) contra Postgres real con migraciones y seed. |
| 24 | **TASK-301: el esqueleto del POS** | **Cerrada el 2026-09-14** | `plna.md` §5, **FASE 3** (la primera). `src/modules/pos/` con las cuatro capas **y contenido real** (el contrato de módulos exige capas con archivos, A-13): `domain/pos-draft.ts` (el borrador del mostrador: agregar —sumando cantidades del mismo producto con la misma nota—, cambiar cantidad, quitar, subtotal informativo con `roundCurrency` y las validaciones que devuelven `PosError`), `ports/pos-catalog.ts` (productos vendibles del local con el precio **ya resuelto** por `getPublicMenu`/`apply-location-pricing`), `adapters/in-memory-pos-catalog.ts` (doble completo del puerto) y `features/search-pos-catalog/` (filtra por nombre o categoría reusando `normalizeSearchText`, con **una sola** lectura del catálogo por búsqueda). **22 tests nuevos** y el mapeo de `PosError` en `src/shared/lib/http/error-response.ts` (403 incluido, para el POS apagado de TASK-308) con su caso. **Sin dependencias nuevas, sin tocar `orders`** (cero duplicación de `Order` y de `calculateOrderTotals`) **y sin UI**: eso es TASK-302. **Cinco decisiones del owner antes de TASK-303 en adelante** (escritas en `plna.md` FASE 3): (1) datos del cliente en una venta de mostrador —`createOrder` hoy pide nombre y WhatsApp—; (2) orden del cobro (crear el pedido y después registrar `Payment`, o al revés); (3) si se exige un turno de caja abierto para vender; (4) TASK-306 polling cada 3 s (recomendado) o SSE; (5) TASK-307 impresión del navegador (recomendado, sin dependencia) o impresora de red ESC/POS (dependencia nueva + hardware). |
| 25 | **TASK-302: la pantalla del punto de venta** | **Cerrada el 2026-09-14** | `plna.md` §5, FASE 3. `/admin/pos` (servidor: `canUsePOS` + alcance por sucursal reusando `resolveOrderLocationScope`, A) con el cliente `pos-client.tsx`: catálogo del local, búsqueda **en memoria** (el catálogo se trae una vez por local; escribir no dispara una consulta por tecla) y el borrador con cantidades, subtotal y estados vacíos. **El cobro no existe todavía, a propósito** (TASK-303): no hay botón de cobrar, porque un botón que no cobra es un control que miente; hay un caso de E2E que lo fija. La API `GET /api/admin/pos/catalog` solo orquesta (permiso 403 para cocina, 400 sin local, 403 si el local está fuera del alcance del staff) y la composición con Prisma vive en `adapters/production-pos-catalog.ts` —**el guardrail de rutas la atajó con 62 líneas y quedó en 49**—; el filtro se movió a `domain/search-pos-products.ts` para que la pantalla y el caso de uso compartan una sola regla. **+26 tests** (42 en el módulo y la pantalla, 6 en la ruta) y **E2E nuevo de 3 casos**: 375 px con el blanco táctil medido, 1280 px en dos columnas y **cocina redirigida a comandas**. |
| 26 | **Decisiones del owner para la FASE 3 (2026-09-14)** | **Contestadas** | Se preguntaron las cinco decisiones abiertas y el owner amplió el alcance de tres tareas. **Queda así**: (1) la venta de mostrador **pide nombre y número** (correo opcional): no hay "walk-in" sin datos; (2) en el mostrador **se pide y se paga de una vez** (una acción crea la venta y registra el cobro); (3) **apertura y cierre de caja todos los días**, con el monto de apertura y **conteo por denominación de billete en cada moneda** (córdobas, dólares, tarjeta…); (4) el **tipo de cambio** va en una **casilla de configuración** (no por venta ni por turno) y hasta que esté cargado un pago en otra moneda se rechaza; (5) **polling cada 3 s** para el POS; (6) el recibo se genera como **imagen JPG** y se ofrece **enviar** (hoja de compartir del sistema) o imprimir/descargar, **al cobrar y desde el detalle**, **sin API de WhatsApp** por ahora. **TASK-303 se partió en 303a** (moneda y tipo de cambio configurables) **y 303b** (el cobro en el POS), y **TASK-305 quedó ampliada** al arqueo por denominación y moneda: todo escrito en `plna.md` FASE 3. |
| 27 | **TASK-303a: moneda y tipo de cambio configurables** | **Cerrada el 2026-09-14** | `plna.md` §5, FASE 3 (primer corte de la 303, partida por tema). `BusinessSettings.usdExchangeRate` (nuevo, nullable, migración aditiva `20260914231219_add_usd_exchange_rate`, **sin drift** en base limpia), validación zod (positivo, tope 100000 para atajar el dedazo), mapeo en el adaptador de Prisma y campo nuevo en `/admin/settings` («Tipo de cambio del dólar»). El dominio vive en `src/modules/pos/domain/payment-conversion.ts`: un cobro en la moneda del negocio queda igual (redondeado) y uno en dólares se convierte con la tasa; **sin tasa cargada se rechaza con el motivo** (no se inventa un número) y una moneda distinta de NIO/USD se rechaza con su código. `currencyCode` ya existía: la tarea no lo tocó. **El guardrail de docs obligó a tocar `DESIGN_SYSTEM.md`** (cambia `schema.prisma`): quedó escrita la regla «no todo lo configurable es un token». **+7 tests** y **E2E nuevo `admin-exchange-rate.spec.ts`**: guarda 36.5, **recarga la pantalla y lo ve** —la clase de bug de T11, cuando el adaptador de Prisma no escribía `paymentMethod` y el unitario pasaba igual—, comprueba que 100001 no se guarda y deja la casilla como la encontró. |
| 28 | **TASK-303b: el cobro de la venta de mostrador** | **Cerrada el 2026-09-14** | `plna.md` §5, FASE 3. Migración aditiva `20260914233611` (`Order.customerEmail` + `Payment.currency`, **sin drift**). `registerPosSale` cobra **en un solo paso** (decisión del owner): llama a `createOrder` —la única puerta de precios y totales— y después registra los cobros con su **moneda** (el arqueo necesita saber si en el cajón hay córdobas o dólares); el cambio se **deriva** con `calculateOrderChange`. **Dos controles de plata**: antes de crear nada compara lo cobrado contra el total del borrador —misma fórmula— y después contra el total real (409 con el número de pedido y la diferencia si el menú cambió). `PosCatalogProduct` sumó `packagingFeeAmount` y la pantalla muestra subtotal / empaque / **total** (antes solo el subtotal: el empaque aparecía recién en la comanda). UI: nombre + número + correo opcional, método en dos chips, moneda **solo si hay tasa cargada**, monto y «Cobrar» con el total en la etiqueta; la clave de idempotencia se renueva solo cuando la venta sale bien. **El guardrail de rutas la atajó con 64 líneas** → el mapeo se movió a `sale-payload.ts` y quedó en 45. **+36 tests** (**287 archivos / 1954**) y **E2E del POS 4/4**: cobra de verdad y el pedido aparece en comandas; **suite E2E local completa 103 / 6 / 0**. Decisiones anotadas en `plna.md`: el mostrador **no** aplica el gate operativo y **no** exige turno abierto todavía (TASK-305 atribuye los cobros por ventana de tiempo). |
| 29 | **TASK-304: la caja ve lo que cobró el mostrador** | **Cerrada el 2026-09-14** | `plna.md` §5, FASE 3. El alta desde el POS ya existía (303b) y el pedido ya llegaba al tablero: el hueco era que **la plata no se veía**. `registerPosSale` registra los cobros en `Payment`, pero el detalle del admin solo mostraba `Order.paidWithAmount` (T12), que un pedido del POS **no** setea → ahora `getOrder` trae los cobros y el detalle los muestra («Cobrado en el mostrador») con su **medio y su moneda**: un cobro de US$3 se muestra `USD 3.00` y no con el símbolo del negocio, que sería un número falso. El **cambio no se recalcula** (se derivó al cobrar y la tasa puede haber cambiado). **+4 tests** y **el E2E del POS cubre el recorrido completo**: cobra → aparece en comandas → **se acepta desde la fila** (avanza con las mismas reglas que un pedido del checkout) → el detalle muestra el cobro. Línea de base: **287 archivos / 1958 tests**, **E2E local 103 / 6 / 0**. |
