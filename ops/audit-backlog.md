...
> **A-17, A-19, A-20 y A-23 son decisiones de producto o de operación: no se implementan sin
> respuesta del owner** (**A-15 salió de esa lista el 2026-09-25**: el owner decidió cómo se corrige un
> cobro y quedó cerrada con `A-58` y `A-59`). **A-16, A-18 y A-22 son trabajo técnico** ya acotado. **A-21 se cerró el
...
| A-15 | **CERRADO** (2026-09-25, TASK-AUD-059): **un cobro de un pedido cancelado seguía contando en el arqueo**. `listPaymentsInRange` filtraba por local y ventana **sin mirar el estado del pedido** y `update-order-status.ts` no tocaba pagos. Hoy el problema tiene las tres salidas que le faltaban, cada una en su camino: cancelar un pedido **cobrado** deja la **devolución pendiente** (Bloque 3.5) y su aprobación resta del arqueo el efectivo que salió del cajón (`Refund`, Bloque 3); un cobro **mal registrado** se **anula** conservando la fila con cuándo, quién y por qué, y sale del arqueo, del saldo del pedido y de la conciliación (`A-59`, migración `20260925120000_add_payment_void`); y las métricas comerciales nunca cuentan plata devuelta (`A-58`). Los **datos históricos** no se reparan (no autorizado) | decisión + bug de plata | **P1** | `cerrado` (2026-09-25) | rama `docs/task-aud-059-cierre` |
...
| A-60 | **El detalle de la orden le muestra la plata a cocina**: `/admin/orders/[id]` dibuja montos, PIN de retiro, cobros y el bloque de factura para el rol `kitchen`, que por regla del repo **no maneja plata** (`AGENTS.md` § *El proyecto*). La tarjeta del KDS sí filtra el dinero del salón; el detalle no. Falta puerta de dominio (familia de `canCollectPayment`) y, si la API le devuelve los montos a cocina, filtrado en el servidor: ocultar en React no es autorización | bug (fuga de dato a un rol) | P2 | `reportado` (discovery de `TASK-ORDERS-001`) | — |
| A-61 | **`GET /api/admin/orders` proyecta de más y filtra de menos**: la respuesta incluye `orderLookupTokenHash` (el token con el que el cliente consulta su pedido) y los campos de GPS (`customerLat`/`customerLng`/`geoAccuracy`/`geoCapturedAt`) que ninguna vista de la bandeja lee; además **ignora** `lateOnly` y `limit`, y no pagina — con miles de pedidos la pantalla los trae todos | deuda / seguridad (proyección excesiva) | P2 | `reportado` (discovery de `TASK-ORDERS-001`) | — |
| A-62 | **El filtro de estado no vive en la URL como los demás**: `statusFilter` es estado de React (`onStatusChange={setStatusFilter}`) mientras búsqueda, local, tipo, pago y modo cocina se guardan en `comanda-url.ts`. Recargar o compartir el enlace pierde el filtro, y la spec de Órdenes da por sentado que los filtros van en la URL | UI / inconsistencia | P3 | `reportado` (discovery de `TASK-ORDERS-001`) | — |
| A-63 | **Las fechas del detalle usan la zona del navegador**: `/admin/orders/[id]` formatea con `toLocaleString()` sin zona, mientras la lista usa la zona del negocio. El mismo pedido muestra horas distintas según desde dónde se abra el panel | bug (presentación de datos) | P3 | `reportado` (discovery de `TASK-ORDERS-001`) | — |
| A-64 | **Cinco mapas estado→etapa y cuatro formateadores de tiempo duplicados** en Órdenes (`comandaLane`, `orderBucket`, `getAdminOrderSolidStatus`, `ORDER_JOURNEY`, `DISPATCHED_STATUSES`), más `scheduledForAnotherDay` resolviendo con `includes` dentro de un bucle (O(n²)): una regla del dominio escrita cinco veces | deuda | P3 | `reportado` (discovery de `TASK-ORDERS-001`) | — |
| A-65 | **Alias históricos en la sección Órdenes**: `text-st-*` en 9 archivos, alias de color viejos (`bg-danger`, `border-border`, `--accent` en sombra) y dos `<select>` crudos. El techo de deuda los permite hoy; bajarlo es trabajo de la sección, no de una TASK transversal | deuda / UI | P3 | `reportado` (discovery de `TASK-ORDERS-001`) | — |
| A-66 | **El cajero ve la entrada Órdenes y recibe 403**: el menú se la muestra a todo rol, la API le responde 403 y la pantalla le dice «Sesión de administrador requerida» (que miente: hay sesión, falta permiso). Las dos salidas —sacarle la entrada y que su inicio sea el POS, o darle un modo acotado de Órdenes— son **producto**: la spec de Órdenes lo deja escrito y no lo decide | decisión | P2 | `decisión-pendiente` (owner) | — |
...
| A-59 | **CERRADO** (2026-09-25, TASK-AUD-059): **Void/Reversal de cobro** (alcance remanente de A-15). Un `Payment` mal registrado —duplicado, con el monto o el medio equivocados— se **anula** conservando el registro original: migración aditiva `20260925120000_add_payment_void` (`voidedAt`, `voidedByUserId`, `voidReason`), puerta `canVoidPayment` (**solo owner**), ruta `POST /api/admin/payments/[id]/void` con su composición, motivo obligatorio y asiento `payment.void`. La regla «un cobro anulado no cuenta» vive en **un solo lugar** por adaptador (`NOT_VOIDED` / `activePayments`): las cinco consultas de lista y la agregación del saldo excluyen los anulados, así que el arqueo, el saldo del pedido, la conciliación y los documentos quedan afuera sin tocar cada consumidor. Y las dos puntas de la misma invariante se rechazan entre sí: no se anula un cobro con devolución viva y no se aprueba la devolución de un cobro anulado. Las **métricas no se tocaron**: no leen `Payment` y el neto de `A-58` ya contemplaba el «importe invalidado» | dinero | P1 | `cerrado` (2026-09-25) | rama `fix/task-aud-059-void-payment` |
| A-57 | **El backup programado de producción no genera archivos**: la config está `enabled: true` con cron `0 9 * * *` y carpeta `oneburguer`, pero las **únicas** acciones de backup del servicio `oneburguer-postgres` son del **2026-09-12** (las dos del drill, `done`). No hay archivo del 13 al 25/09, ni el del día del release a las 09:00, y **no hay retención declarada**. Los datos posteriores al 12/09 (pedidos, cobros, turnos, facturas) no están respaldados: el backup del release se generó a mano. **Requiere revisar la sección Backups del servicio en Easypanel** (hallazgo del preflight del release de AUD-003..006). **Actualización 2026-09-26**: el owner confirmó un **backup manual de la base** para el release de `SCREEN-POS-QUICK-SALE-001`; **el hallazgo sigue abierto** porque lo que falla es el **scheduler** y no hay retención declarada. Desde el entorno del agente **no se pudo verificar el archivo**: el panel de Easypanel responde **401** sin token (acá no está `EASYPANEL_TOKEN`) | operación / datos | **P1** | `reportado` (release 2026-09-25) | — |
...
| A-55 | **CERRADO** (2026-09-25): dos cobros simultáneos del mismo pedido (`register-order-payment`): el tope se valida leyendo la suma de los cobros y después escribiendo, sin lock del pedido; dos requests podrían pasar los dos y cobrar de más | bug (carrera / dinero) | P2 | `reportado` (TASK-AUD-005) | — |
| A-51 | **Arqueos firmados por el cierre imperfecto (datos previos a TASK-AUD-005)**: el defecto de las tres escrituras sueltas y el esperado sin movimientos ni devoluciones. **Pasada read-only del 2026-09-25 (API de admin, solo `GET`)**: los **5** cierres de producción tienen su `expectedAmount` firmado, **4** con su conteo (el quinto es un **cierre ciego**, sin conteo por diseño) y **ninguno de los 5 tenía retiros ni devoluciones** (el cambio de fórmula de AUD-005 no altera ningún número histórico). **Cero confirmados, cero probables**; el detalle de `ShiftBankClose` queda **no determinable** porque producción no tiene bancos configurados | dato | P2 | `no-repro` (2026-09-25) | — |
| A-50 | **Ventas de mostrador anteriores al arreglo de atomicidad**: el bug que cerró TASK-AUD-004 pudo dejar pedidos con menos `Payment` que los declarados (o con **cero**) y cupones consumidos por ventas que no se cobraron. El fix garantiza la invariante **de acá en adelante** y **no** detecta ni repara lo anterior (no hay backfill ni migración a propósito). Una venta parcial vieja **subcuenta la caja** de su turno. Requiere una consulta de solo lectura y una **decisión del owner**. **Pasada read-only del 2026-09-25**: no identificable con certeza desde la API del admin (el listado no expone los cobros ni la clave de intento); requiere leer la base | dato / decisión | P2 | `decisión-pendiente` (owner) | — |
...
### A-15 · Un cobro de un pedido cancelado sigue contando en el arqueo — `cerrado` (2026-09-25, TASK-AUD-059)

- **Qué era**: el arqueo del turno traía los cobros por local y ventana de tiempo y **no miraba el estado del
  pedido**; `update-order-status.ts` no tocaba pagos y no había ninguna forma de registrar la plata devuelta.
  Si se cobraba en el mostrador y después se cancelaba el pedido, el esperado **seguía contando esa plata**; si
  el cajero la devolvía, el cierre marcaba **faltante** sin documento que lo explicara.
- **Cómo se cerró** (las tres salidas, cada una en su camino):
  1. **La plata que sale del cajón**: cancelar un pedido **cobrado** deja la **devolución pendiente**
     (Bloque 3.5) y, al aprobarla el dueño, el efectivo devuelto **resta** del arqueo del turno
     (`Refund` + `refundsTotalInBusinessCurrency`).
  2. **El registro que no debía existir**: un cobro mal cargado se **anula** (`A-59`) conservando la fila con
     cuándo, quién y por qué. La regla «un cobro anulado no cuenta» vive en un solo lugar por adaptador, así
     que sale del arqueo, del saldo del pedido, de la conciliación y de los documentos sin tocar cada
     consumidor.
  3. **Lo que no es ingreso**: las métricas comerciales nunca cuentan plata devuelta o invalidada
     (`A-58`, `netOrderValue` / `countsAsSale`).
- **Evidencia**: `close-shift.postgres.test.ts` (el cierre con devoluciones), `admin-overview-net-revenue.test.ts`
  (6 casos + mutación) y `void-payment.postgres.test.ts` (4 casos contra PostgreSQL real: el arqueo pasa de 800 a
  300 al anular el cobro de 500 con la fila original intacta, el saldo del pedido vuelve a 0, dos anulaciones
  simultáneas firman una sola, y una devolución viva bloquea la anulación). Las tres mutaciones verificadas.
- **Lo que queda fuera a propósito**: los **datos históricos** no se reparan ni se backfillean (no autorizado);
  lo que quedó sin determinar es `A-50` (ventas POS parciales previas a AUD-004) y `A-51` (cierres firmados por
  el cierre imperfecto), cada uno con su ficha. Un turno **ya cerrado** no se recalcula: es un documento firmado.
...
| **Release AUD-003..006** (sin fila `A-xx`: es el despliegue de las cuatro TASK) | **AUD-003** (arqueo ciego: sumandos y traspaso), **AUD-004** (venta del mostrador atómica), **AUD-005** (cierre de turno atómico con lock) y **AUD-006** (correlativo de factura asignado, no leído) llevados a **producción** con Easypanel: `main` `17ecb27` → `build-20260925-123054`, 2026-09-25 12:29 UTC | rama `docs/release-aud-003-006` | Preflight read-only (servicio, réplicas, Postgres sin puerto expuesto, hosts), **backup pre-deploy** `oneburger/2026-09-25T12:27:16.589Z.sql.gz` (`done`), `commit.sha` del panel = `17ecb27`, `/api/health` = `build-20260925-123054`, `/api/readiness` `ready` (DB 11 ms), smokes **menú 7/7** y **hosts 6/6**, y HTTP de solo lectura (`/admin/cash`, `/admin/pos`, `/admin/history/cierres` → 307 al login; `/api/admin/pos/shift/x` y `/api/admin/orders` → 401; `/api/menu` y `/api/locations` → 200). Sin migraciones nuevas, sin tocar otros servicios y **sin reparar datos históricos** |
