...
| A-59 | **Void/Reversal de cobro (alcance remanente de A-15, sin implementar)**: registrar la invalidacion de un `Payment` **conservando el registro original** (`Payment.voidedAt`, `voidedByUserId`, `voidReason` con **migracion aditiva**), capability de dominio `canVoidPayment` (**solo owner**), su ruta y composicion, el rechazo con motivo obligatorio y la **exclusion del importe invalidado** en el arqueo y en las metricas. Los `Refund` cubren la devolucion real de plata; esto cubre «el cobro se registro mal y se invalida». La semantica economica neta de `A-58` (cerrada) **ya lo contempla** como «importe invalidado», asi que las metricas no se tocan: solo falta el camino que lo registra. Verde/fixture: `Payments` de produccion sin campos de void; nada que reparar | dinero | P1 | `reportado` (2026-09-25, alcance de A-15) | — |
