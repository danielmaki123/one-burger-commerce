...
| **Factura simple (ítem 4 de la ronda)** (2026-09-18) | **El documento que se lleva el cliente — NO fiscal**: modelo `Invoice` (migración `20260918140000`, una por pedido, número correlativo `F-000001`, datos del negocio y del cliente **congelados** al emitir, montos congelados) + `legalName`/`taxId` en `BusinessSettings` (razón social y RUC del negocio) y en `Customer`. `emitInvoice` (se factura lo que **se cobró**: sin cobros 409, cancelado 409; volver a pedirla devuelve la misma), `getOrderInvoice`, `GET/POST /api/admin/orders/[id]/invoice` (entrega el documento quien cobra: `canUsePOS`), la sección **Factura** en el detalle del pedido y el papel `invoice-sheet.ts` impreso con la hoja del sistema (el navegador lo guarda como PDF: misma decisión que 1.6, **sin dependencia nueva**) | este mismo commit | `tarea-factura-simple-*.png`, `tarea-factura-emitida-*.png`, `tarea-factura-impresa-*.png` |
