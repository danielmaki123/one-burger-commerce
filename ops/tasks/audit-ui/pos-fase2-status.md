...
| **Rediseño de la factura: hoja A4** (2026-09-18) | La factura era texto monospace sin logo ni sucursal. Ahora es una **hoja A4** en su propia página (`/admin/orders/[id]/invoice/print`, HTML con CSS embebido, sin popup ni iframe ni dependencia nueva): logo del negocio (o isotipo, o solo el nombre), bloque de negocio con razón social + RUC + dirección y teléfono fiscales, **sucursal de retiro congelada al emitir** (migración aditiva `20260918160000` con `branchName/AddressLine/City/Phone/Whatsapp/MapsUrl`; las facturas viejas salen sin el bloque), cliente, tabla de ítems con jerarquía, totales con el TOTAL destacado, **pago real** (efectivo con «Pagó con»/«Vuelto», tarjeta, transferencia, cobro partido y la propina aparte) y pie «Documento no fiscal» con el número de pedido. `Ctrl+P` sale en A4 (2 cm arriba/abajo, 1,5 cm a los costados) con el panel del admin fuera del papel | este mismo commit | `tarea-factura-a4-1280.png`, `tarea-factura-a4-375*.png`, `tarea-factura-a4.pdf` (el PDF real) vs `tarea-factura-antes-texto-*.png` |
