...
| **T5** | **Carrito + checkout** (absorbe `TASK-checkout-v2`) — **cerrada** en sus fases 1, 2, 3, 6 y 7: `pickupMaxMinutes` con migración y validación cruzada, franja "listo entre X y Y" en checkout y confirmación (la hora guardada sigue siendo el mínimo), dirección del local en el punto de retiro, prefijo de WhatsApp derivado del teléfono del negocio, edición por ítem del carrito verificada y **vista previa en vivo de los turnos y la última orden en `/admin/settings`** (fase 2). **Quedan pendientes** las decisiones D1/D2 | `business-settings.schema.test.ts` → `pickupMaxMinutes`; `pickup-slots.test.ts` → rango; `pickup-preview-helpers.test.ts`; `checkout/page.test.tsx`; `settings-client.test.tsx` | **Sí** (migración `add_pickup_max_minutes`) |
