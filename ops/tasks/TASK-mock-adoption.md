...
| **T9** | **Promos por cantidad** (B2G1) — **motor cerrado**: `CouponType.bogo` con `buyQuantity`/`freeQuantity` y alcance (`all`/`category`/`subcategory`/`product`), descuento calculado por el servidor y validación de la config. **Faltan**: el campo del código en el checkout y la pantalla del admin para crear promos (sin eso el owner depende de la base) | `promo-bogo.test.ts` · `create-order.test.ts` | Medio |
