...
**Estado:** plan **aprobado** (D-A, D-B y D-C resueltas el 2026-09-12) · **programa CERRADO el
2026-09-12**: ola 1 completa (T1 a T7, incluida T3.1) y ola 2 completa salvo **T10 (favoritos), que
sigue bloqueada** por falta de login de cliente real. Incluye **T8 (multi-sucursal) cerrada en sus 7
fases** y **T9, T11, T12 y T13 cerradas**. El detalle de qué quedó abierto está en
`ops/project-state.md` §4 · **Fecha:** 2026-09-12 · **Origen:** pedido del owner sobre
...
| **T5** | **Carrito + checkout** (absorbe `TASK-checkout-v2`) — **cerrada** en sus fases 1, 2, 3, 4, 6 y 7: `pickupMaxMinutes` con migración y validación cruzada, franja "listo entre X y Y" en checkout y confirmación (la hora guardada sigue siendo el mínimo), dirección del local en el punto de retiro, **pedidos para días futuros con selector de día** (fase 4, D1 = sí), prefijo de WhatsApp derivado del teléfono del negocio, edición por ítem del carrito **cubierta con test** y **vista previa en vivo de los turnos y la última orden en `/admin/settings`** (fase 2). **La fase 5 (presets de propina) quedó descartada** (D2 = no) | `business-settings.schema.test.ts` → `pickupMaxMinutes`; `pickup-slots.test.ts` → rango; `pickup-days.test.ts` → días futuros; `cart-editing.test.tsx`; `pickup-preview-helpers.test.ts`; `checkout/page.test.tsx`; `settings-client.test.tsx` | **Sí** (migración `add_pickup_max_minutes`) |
...
|---|---|---|---|
| **T8** | **Multi-sucursal** — **alcance aprobado el 2026-09-12 (D-T8): menú y precios por local**. **CERRADA en sus 7 fases** (commits `02b8f07` … `1c427d4`): modelo `Location` + `LocationProduct` + `Order.locationId` con backfill, API y pantalla `/admin/locations`, catálogo y precios por local, menú público por local, selector de local en el checkout con el gate operativo por local, operación por local (bandeja, detalle, ticket, confirmación e historial) y el estado operativo fuera de `/admin/settings`. El brief con el detalle es `ops/tasks/TASK-multi-location.md` | **Alto** (migración + admin + operación) |
...
> **Orden y dependencias:** **la ola 2 está cerrada** — T8 (multi-sucursal, 7 fases), T9, T11, T12 y
> T13 hechas. Lo único que queda es **T10 (favoritos), bloqueada** hasta que exista login de cliente
> real (el OTP responde 503 en producción). Los pendientes que no son del programa (contenido del
> menú, notificaciones, backups) están en `ops/project-state.md` §4.
