...
T2 (home), T3 (menú) y T4 (producto) cerradas**; sigue **T3.1 (color por categoría)** y después
...
| **T3.1** | **Color por categoría** (pedido del owner el 2026-09-12): campo de color en `Category`, editable en `/admin/menu`, y las tarjetas del menú y los chips de la home pintados con él. **Con control de contraste**: el color lo elige el owner, así que el texto encima tiene que pasar AA (mismo criterio que la paleta de apariencia) | `category-color.test.ts` (nuevo) · `menu-product-card.test.tsx` | **Sí** (migración) |
