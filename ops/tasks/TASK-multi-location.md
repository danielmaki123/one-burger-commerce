...
| **2** | Casos de uso y API de locales (`/api/admin/locations`): CRUD, "no se puede borrar el último", copiar catálogo del primario al crear · **2a CERRADA** (reglas de validación + `create/update/delete-location` con sus tests) · **2b pendiente** (rutas de API). La **copia del catálogo** se hace en la fase 4: hasta que existan filas de `LocationProduct` (fase 4), copiar sería copiar nada | `location-validation.test.ts` · `create-location.test.ts` · `update-location.test.ts` · `delete-location.test.ts` · `route.test.ts` |
