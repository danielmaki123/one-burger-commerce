...
| **2** | Casos de uso y API de locales (`/api/admin/locations`): CRUD, "no se puede borrar el último", copiar catálogo del primario al crear | `create-location.test.ts` · `update-location.test.ts` · `delete-location.test.ts` · `route.test.ts` |
